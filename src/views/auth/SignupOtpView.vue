...
const phoneNumber = computed(() => {
  return (
    route.query.phone ||
    store.pendingSignup?.phoneNumber ||
    ''
  )
})
...
const resendOtp = () => {
   ...
   * بعداً:
   *
   * await authApi.sendOtp(phoneNumber.value)
   */
...
<template>
  <main
    dir="rtl"
    class="min-h-screen flex items-center justify-center relative overflow-hidden bg-slate-100 px-4 py-8 sm:px-6 lg:px-8"
  >
    <!-- Background -->
    <div
    ...
    >
      <section
        class="w-full rounded-2xl bg-white/90 backdrop-blur-md shadow-xl p-5 sm:p-7 md:p-8"
      >
        <!-- Icon -->
        ...
        <div
          v-if="errorMessage"
          class="mt-4 rounded-lg bg-red-50 px-3 py-2.5 text-center text-sm text-red-600"
        >
          {{ errorMessage }}
        </div>
        ...
        <div
          v-if="successMessage"
          class="mt-4 rounded-lg bg-green-50 px-3 py-2.5 text-center text-sm text-green-600"
        >
          {{ successMessage }}
        </div>
        ...
        <button
          ...
          "
          class="mt-6 w-full rounded-xl bg-green-600 py-3.5 text-sm sm:text-base font-semibold text-white transition hover:bg-green-700 disabled:cursor-not-allowed disabled:opacity-50"
          @click="verifyOtp"
        >
          {{
            isVerifying
        ...
        <button
          type="button"
          class="mt-5 block w-full text-center text-sm text-slate-500 transition hover:text-slate-700"
          @click="
            router.push({
              name: 'signup',
            })
        ...
        <!-- Development OTP -->
        <div
          class="mt-6 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-center text-xs leading-5 text-amber-700"
        >
          کد آزمایشی:
